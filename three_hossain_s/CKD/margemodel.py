...
def enhanced_feature_selection(X, y, feature_names, dataset_name):
    if BORUTASHAP_AVAILABLE:
        return borutashap_feature_selection(X, y, feature_names, dataset_name)
    else:
        return fallback_feature_selection(X, y, feature_names, dataset_name)
...
for dataset_name, config in datasets.items():
    ...
    # Apply BorderlineSMOTE to the entire preprocessed dataset
    ...
    print(f"{'='*50}")
    print(f"Dataset shape before resampling: {X_preprocessed.shape}")
...
print("Both datasets were processed using ColumnTransformer and Pipeline to prevent data leakage, with mean imputation for numerical columns, "
      "mode imputation for categorical columns, attribute-wise encoding, attribute-wise scaling, and BorderlineSMOTE applied before train-test splitting for 50-50 balance. "
      ...
      "Key predictors drive CKD detection with high accuracy. BorderlineSMOTE was used instead of SMOTE to better handle borderline cases in the imbalanced datasets.")